...
public class SafeSqlPolicy {
    ...
    public String sanitizeSelect(String sql) {
        ...
        // Normalize: trim and remove a single trailing semicolon if present
        String s = sql.trim();
        if (s.endsWith(";")) s = s.substring(0, s.length() - 1).trim();

        // Still forbid multiple statements (any additional semicolons inside the text)
        if (s.indexOf(';') >= 0) bad("Multiple statements are not allowed");

        if (!SELECT_OR_WITH_START.matcher(s).find()) bad("Only SELECT/CTE statements are allowed");
        if (FORBIDDEN.matcher(s).find()) bad("Statement contains forbidden keywords");
        ...
        if (!LIMIT_PATTERN.matcher(s).find()) {
            s = s + "\nLIMIT 5000";
        }
        return s;
    }
